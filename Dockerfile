...
RUN if [ $(uname -m) = "x86_64" ]; then export ARCH=x86_64; else export ARCH=aarch64; fi && \
    cd /opt && \
    wget https://github.com/zephyrproject-rtos/sdk-ng/releases/download/v0.17.1/zephyr-sdk-0.17.1_linux-${ARCH}_minimal.tar.xz && \
    tar xf zephyr-sdk-0.17.1_linux-${ARCH}_minimal.tar.xz && \
...
RUN mkdir /ncs
RUN cd /ncs && west init -m https://github.com/nrfconnect/sdk-nrf --mr v3.1.0
